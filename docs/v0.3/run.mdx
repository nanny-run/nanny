...
Every run emits [NDJSON events](/v0.3/concepts/event-log) to stdout. `ExecutionStarted` is always first; `ExecutionStopped` is always last:
...
For per-function governance (marking individual tools and rules in code), see the [Rust SDK guide](/v0.3/guides/rust-sdk) or [Python SDK guide](/v0.3/guides/python-sdk).
