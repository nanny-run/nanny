...
Cloud sync has no config block. It turns on wherever the `NANNY_API_KEY` environment variable is set, and there is no field to flip. No key, endpoint, or org lives in `nanny.toml` — it is committed, and a committed file must never hold a secret or differ between your staging and production deploys.
