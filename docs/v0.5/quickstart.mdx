...
For upgrade paths, uninstall, and troubleshooting on each platform, see [Installing Nanny](/v0.5/install).
...
`nanny init` also writes `.nanny/app.toml`, a permanent, one-time identity for this app (an `app_id` plus a name you'll be prompted for). It's meant to be committed alongside `nanny.toml`.
...
if you want a different filename, see [Event Log](/v0.5/concepts/event-log) for details.
