...
| `--join=<appId>`  | string | —              | Join an existing governance server by app id (from that server's `.nanny/app.toml`), instead of enforcing locally. Explicit and app-id-only: never a name, never auto-detected. Not compatible with `--serve`. |
