...
- A dashboard of every run: steps, tokens, stop reasons, models, and which app it came from
...
That is the whole setup. There is no config field, no credential file, no login command, and nothing written to disk — which is what makes it behave identically on a laptop, in CI, on a read-only filesystem, and across twenty replicas sharing one key.
...
```
nanny: mode managed — syncing to https://api.nanny.run (app: gotm-nanny)
...
<Note>
  ...
  printed too — it is not treated as success.
</Note>

## When the cloud is unreachable

Nothing is lost and nothing blocks. A batch that cannot be delivered is retried with backoff, and if it still cannot be sent it is written to a durable outbox under `.nanny/spool/` and delivered on the next run, oldest first.
...
The outbox is capped (64 MB) and gitignored, like the logs beside it.
