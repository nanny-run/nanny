...
For upgrade paths, uninstall, and troubleshooting on each platform, see [Installing Nanny](/v0.7/install).
...
See [Tool labels](/v0.7/concepts/tool-labels) for all five.
...
Your own source is untouched. See [Rule packs](/v0.7/guides/rule-packs).
...
Events go to stdout and Nanny's own output goes to stderr, so a redirection
gives you the log and nothing else:

```bash
nanny run > events.ndjson
```

In a container you redirect nothing: the runtime already collects stdout, which
is where every log shipper reads it from. See
[Event Log](/v0.7/concepts/event-log).
