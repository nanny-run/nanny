...
The Python SDK brings the same enforcement model as the [Rust SDK](/v0.7/guides/rust-sdk) to Python, `@tool`, `@rule`, and `@agent` decorators that govern each call before it executes.
...
See [Long-lived processes and NannyStop](/v0.7/guides/governance-server#long-lived-processes-and-nannystop) for the full explanation of why this only applies under `nanny run`/`--join`.
...
For cross-process and cross-machine enforcement, use the [governance server](/v0.7/guides/governance-server).
...
Rule packs ship the same shape as the rules above. See [Rule packs](/v0.7/guides/rule-packs).
