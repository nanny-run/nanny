...
without being written for it. See [Tool labels](/v0.7/concepts/tool-labels).
...
```bash
nanny run   # starts a governor, runs [start].cmd underneath it
```

**Rust SDK**, for Rust agents. Annotate functions with `#[nanny::tool]`, `#[nanny::rule]`, and `#[nanny::agent]` for per-call enforcement and custom rules. See the [Rust SDK guide](/v0.7/guides/rust-sdk).

**Python SDK**, the same model as decorators: `@tool`, `@rule`, `@agent`. Works with LangChain, CrewAI, or any Python agent framework. See [Python SDK](/v0.7/guides/python-sdk).

**Rule packs**, curated sets of rules installed with one command and pinned to a version. See [Rule packs](/v0.7/guides/rule-packs).

**Nanny Cloud**, durable signed audit trails, cost attribution across your fleet, and team access control. The runtime stays unchanged and works without it. See [Connect to Nanny Cloud](/v0.7/guides/managed-mode).
...
<CardGroup cols={2}>
  <Card title="Quickstart" icon="bolt" href="/v0.7/quickstart">
    Install nanny and run your first governed agent in under five minutes.
  </Card>
  <Card title="Installation" icon="package" href="/v0.7/install">
    Install, upgrade, and uninstall on macOS, Linux, and Windows.
  </Card>
  <Card title="How it works" icon="cpu" href="/v0.7/concepts/how-it-works">
    Understand the enforcement model and passthrough mode.
  </Card>
  <Card title="Limits" icon="shield" href="/v0.7/concepts/limits">
    Learn how tool permission and rules are enforced.
  </Card>
  <Card
    title="nanny.toml reference"
    icon="file-text"
    href="/v0.7/reference/nanny-toml"
  >
  ...
  <Card
    ...
    icon="rust"
    iconType="brands"
    href="/v0.7/guides/rust-sdk"
  >
    Per-function governance with `#[nanny::tool]`, `#[nanny::rule]`,
    `#[nanny::agent]`.
  </Card>
  <Card title="Python SDK guide" icon="python" href="/v0.7/guides/python-sdk">
    Per-function governance with `@tool`, `@rule`, `@agent` decorators. Works
    with LangChain, CrewAI, and any Python agent framework.
  </Card>
  <Card title="Governance server" icon="server" href="/v0.7/guides/governance-server">
