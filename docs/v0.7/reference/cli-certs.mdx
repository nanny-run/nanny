...
**Run these inside the project.** A bundle belongs to one app, identified by the
committed `.nanny/app.json`, and there is no flag to point somewhere else.

## Sandbox and live

Every command targets **sandbox** unless you pass `--live`.
...
Two environments means two certificate authorities. A CA is what a governor
trusts, so sharing one across environments means a client certificate issued
for sandbox is admitted to production: the production governor verifies the
signature, the signature is valid, and nothing distinguishes them. Separate
authorities make that impossible rather than merely discouraged.

Production is typed every time. `rotate` takes no confirmation, and reissuing a
live bundle stops every joiner still holding the old client certificate.

This flag selects the trust anchor and reads nothing else. The
`nny_sdbx_`/`nny_live_` key prefix selects which cloud your evidence reaches.
Same words, two independent axes.
...
```bash
nanny certs generate [--live] [--san <host>]... [--days <days>] [--force]
```
...
~/.nanny/servers/<app_id>/certs/sandbox/
  ...
  ca.key      CA private key. Never leaves the machine that generated it: it
              signs certificates, so whoever holds it can mint a client the
              governor trusts. Needed by nanny certs rotate.
...
`nanny run` reads `server.crt`, `server.key`, and `ca.crt` automatically. Agents on other machines need `client.crt`, `client.key`, and `ca.crt`.

#### Flags

| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--live` | flag | — | Issue from the live authority instead of the sandbox one. |
...
`rotate` requires `ca.key` to be present in `~/.nanny/servers/<app_id>/certs/sandbox/`. This file only exists when `nanny certs generate` created the CA. If `ca.key` is missing, `rotate` exits with an error and suggests `nanny certs import`.
...
Delete all cert files from `~/.nanny/servers/<app_id>/certs/sandbox/`.

```bash
nanny certs remove
```

Prompts for confirmation before deleting. After removal, `nanny run` with a non-loopback address will refuse to start until new certs are generated or imported.
...
The governance server watches `~/.nanny/servers/<app_id>/certs/sandbox/` for file changes. When cert files are updated, by `nanny certs import`, `nanny certs rotate`, or an external PKI agent writing to the directory, the server reloads the new certs without restarting. New connections use the new cert immediately; existing connections complete on the old cert.
