...
# The event log has no field here. Events go to stdout; redirect them where
# you want them. See /v0.7/concepts/event-log.

# Nor does cloud sync. It turns on for any machine that sets the
# NANNY_API_KEY environment variable. There is no endpoint or key here.
```
...
The five booleans are [tool labels](/v0.7/concepts/tool-labels). Rules read
...
[`nanny rules add`](/v0.7/reference/cli-rules).
...
See [Connect to Nanny Cloud](/v0.7/guides/managed-mode) for the full flow, and [Deploying a governed app](/v0.7/guides/deploying) for containers and CI.
