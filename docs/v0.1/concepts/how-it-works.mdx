...
**Scope:** All agents in the diagram run within one process. This covers any framework that orchestrates agents in a single runtime. Cross-process and cross-machine fleet enforcement is the [v0.1.6 cloud layer](https://nanny.run).
...
Step and cost enforcement require the agent to report tool calls — either via the [Rust SDK](/v0.1/guides/rust-sdk) macros or the Python SDK decorators.
