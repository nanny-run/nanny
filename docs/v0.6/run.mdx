...
```bash
# Reads [start].cmd from nanny.toml and governs it
nanny run

# Run a governance server, with the app underneath it
nanny run --serve
```
...
Use it when one set of rules has to cover several processes or machines. A single-process agent is fully governed by plain `nanny run`.
