...
|-----------|------------|----------------------|
| Local enforcement | `NANNY_BRIDGE_SOCKET` or `NANNY_BRIDGE_PORT` is set | A TCP connection succeeds |
| Network server | `NANNY_BRIDGE_ADDR` is set | A TCP connection to the server address succeeds |
| Certificates | both bundles, sandbox and live | Cert files are present and not expired |
...
This means `nanny health` run from a regular terminal (outside `nanny run`) will typically show `local enforcement: not running`, that's expected. These env vars are injected by `nanny run` into the child process, not into the terminal that launched it.
...
- [`nanny status`](/v0.6/run#governance-server), focused status view for the governance server
- [`nanny certs show`](/v0.6/reference/cli-certs), cert expiry and file inventory
