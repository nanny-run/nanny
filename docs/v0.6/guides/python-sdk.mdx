...
The Python SDK brings the same enforcement model as the [Rust SDK](/v0.6/guides/rust-sdk) to Python, `@tool`, `@rule`, and `@agent` decorators that enforce limits per function call.
...
[Named sets share one counter](/v0.6/concepts/limits#named-sets-share-one-counter-they-are-not-separate-budgets)).
