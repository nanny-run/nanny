...
<Note>
  The old `[managed]` block (with `endpoint` / `api_key`) is **retired**. If an
  old `nanny.toml` still has it, Nanny ignores it and prints a one-time
  deprecation notice pointing you at `nanny auth login`.
</Note>
