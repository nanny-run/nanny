...
|-------|------|---------|-------------|
| `mode` | `"local"` \| `"managed"` | `"local"` | `"local"` enforces locally with no network calls. `"managed"` additionally syncs the event log to Nanny Cloud **when you're logged in** (`nanny auth login`). Enforcement stays local either way. See [Connect to Nanny Cloud](/v0.4/guides/managed-mode). |
...
Cloud sync has no config block. It turns on when **both** are true: `[runtime] mode = "managed"` above, and you're logged in via `nanny auth login`. No key, endpoint, or org lives in `nanny.toml` — login handles the credential, so your committed config never holds a secret.
