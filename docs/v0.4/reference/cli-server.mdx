---
title: Governance server (nanny run --serve)
description: Run the Nanny governance server for cross-process and cross-machine enforcement, and manage it with nanny status and nanny stop.
---

The governance server is a long-lived process that enforces Nanny's limits across multiple processes and machines. Agents connect to it over TCP. All tool calls from all connected agents are counted against the same shared budget and step limit.

For single-process agents, use `nanny run` with no `--serve` — no server needed.

<Note>
  These commands were formerly `nanny server start` / `status` / `stop`. Those
  aliases still work but are deprecated and print a notice — use
  **`nanny run --serve`**, **`nanny status`**, and **`nanny stop`**.
</Note>
...
Reads `nanny.toml` from the current directory. Blocks until the server is stopped with `nanny stop` or `CTRL-C`.
...
- `~/.nanny/server.addr` — the bound listen address (used by `nanny status` and `nanny health`)
- `~/.nanny/server.pid` — the server process ID (used by `nanny stop`)
