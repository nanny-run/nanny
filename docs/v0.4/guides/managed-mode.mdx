...
**2. Log in** — this opens your browser to approve:

```bash
nanny auth login
```

Approve, and you're connected. Now run as usual:
...
<Note>
  Sync needs **both**: `mode = "managed"` in the project, and being logged in. A
  local-mode project never syncs even when you're logged in, and a machine that
  never logged in never syncs even in managed mode.
</Note>
