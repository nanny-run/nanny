...
**Nanny CLI** — The universal entry point. Wraps any agent process in any language with zero code changes. Install it once as a system tool and use `nanny run` from any project that has a `nanny.toml` with a `[start]` command configured.

```bash
nanny run                    # reads [start].cmd from nanny.toml
nanny run --limits=researcher   # activates a named limit set
nanny run -- "research topic"   # appends args to [start].cmd
```

**Rust SDK** — For Rust agents, go deeper. Annotate individual functions with `#[nanny::tool]`, `#[nanny::rule]`, and `#[nanny::agent]` to get per-function cost accounting, allowlist enforcement, and custom rules. See the [Rust SDK guide](/guides/rust-sdk).

**Python SDK** *(coming v0.2.0)* — The same model as the Rust SDK, as Python decorators. `@tool`, `@rule`, `@agent`. This is the public launch milestone. See [Python SDK](/guides/python-sdk).

**Nanny Cloud** *(coming v0.3.0)* — Durable audit logs, team dashboards, org-level budget aggregation, and managed enforcement across all your agents. The OSS runtime stays unchanged — Cloud is the observability and coordination layer above it.
...
<CardGroup cols={2}>
  ...
  <Card title="How it works" icon="cpu" href="/concepts/how-it-works">
    Understand the enforcement boundary, the bridge, and passthrough mode.
  </Card>
  <Card title="nanny.toml reference" icon="file-text" href="/reference/nanny-toml">
    Full schema for the configuration file.
  </Card>
  <Card title="Rust SDK guide" icon="code" href="/guides/rust-sdk">
