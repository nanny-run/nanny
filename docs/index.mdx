...
**Nanny CLI** — The universal entry point. Wraps any agent process in any language with zero code changes. Install it once as a system tool and use `nanny run` from any project.

```bash
nanny run python agent.py    # Python agent
nanny run ./my-agent         # Compiled Rust agent
```
