...
The Rust SDK brings Nanny's enforcement boundary into your code. Instead of relying only on process-level limits, you mark individual functions as tools and rules — Nanny then governs each call through the bridge.
...
If your agent runs without `nanny run`, every macro is a no-op. The function executes normally — no bridge call, no overhead:

```bash
# Governed — bridge active, limits enforced (reads [start].cmd from nanny.toml)
nanny run
...
`nanny::http_get` is a built-in tool that routes HTTP GET requests through the bridge. It requires no `#[tool]` annotation — governance is handled entirely bridge-side.
...
In passthrough mode (no `nanny run`), `nanny::http_get` makes the request directly — no bridge call, no overhead.
...
| `last_tool_args` | `HashMap<String, String>` | Arguments of the most recent tool call |
