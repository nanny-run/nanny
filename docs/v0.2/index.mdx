...
Nanny is an open-source enforcement primitive for autonomous agents and multi-agent systems. You tell it how far each agent is allowed to go — in steps, tokens, wall-clock time, and which tools it can touch — and the moment any limit is crossed, Nanny kills the process immediately and emits a structured log saying exactly what happened and why.
...
- It **will not** spend more than your token budget.
...
Nanny sets limits and enforces them. It adds no intelligence — it doesn't interpret your agent's behavior, suggest better limits, or make any decisions. If you set a limit, it holds.
...
- Need hard guarantees that an agent **cannot exceed a token budget or run indefinitely**
...
[limits.analysis]
steps   = 60
tokens  = 200     # tight — this agent makes expensive calls
timeout = 60000
...
**Rust SDK** — For Rust agents, go deeper. Annotate individual functions with `#[nanny::tool]`, `#[nanny::rule]`, and `#[nanny::agent]` to get per-function token accounting, allowlist enforcement, and custom rules. See the [Rust SDK guide](/v0.2/guides/rust-sdk).

**Python SDK** — The same model as the Rust SDK, as Python decorators. `@tool`, `@rule`, `@agent`. Each agent in your fleet gets its own budget ceiling, tool allowlist, and custom rules. Works with LangChain, CrewAI, or any Python agent framework. See [Python SDK](/v0.2/guides/python-sdk).
...
<CardGroup cols={2}>
  <Card title="Quickstart" icon="bolt" href="/v0.2/quickstart">
    Install nanny and run your first governed agent in under five minutes.
  </Card>
  <Card title="Installation" icon="package" href="/v0.2/install">
    Install, upgrade, and uninstall on macOS, Linux, and Windows.
  </Card>
  <Card title="How it works" icon="cpu" href="/v0.2/concepts/how-it-works">
    Understand the enforcement model and passthrough mode.
  </Card>
  <Card title="Limits" icon="shield" href="/v0.2/concepts/limits">
    Learn how timeout, steps, and token limits work.
  </Card>
  <Card
    title="nanny.toml reference"
    icon="file-text"
    href="/v0.2/reference/nanny-toml"
  >
  ...
  <Card
    ...
    icon="rust"
    iconType="brands"
    href="/v0.2/guides/rust-sdk"
  >
    Per-function governance with `#[nanny::tool]`, `#[nanny::rule]`,
    `#[nanny::agent]`.
  </Card>
  <Card title="Python SDK guide" icon="python" href="/v0.2/guides/python-sdk">
    Per-function governance with `@tool`, `@rule`, `@agent` decorators. Works
    with LangChain, CrewAI, and any Python agent framework.
  </Card>
  <Card title="Governance server" icon="server" href="/v0.2/guides/governance-server">
    Cross-process and cross-machine enforcement with shared budget and mTLS.
    New in v0.2.
  </Card>
</CardGroup>
