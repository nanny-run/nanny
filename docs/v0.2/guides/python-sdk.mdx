...
The Python SDK brings the same enforcement model as the [Rust SDK](/v0.2/guides/rust-sdk) to Python — `@tool`, `@rule`, and `@agent` decorators that enforce limits per function call.
...
All fields are live. Before each rule evaluation, the SDK fetches current counters from Nanny’s enforcement layer — `step_count`, `cost_units_spent`, `tool_call_counts`, and `tool_call_history` reflect the actual execution state at the moment your rule runs.
...
- **Loop detection:** the `@rule` fires client-side before Nanny’s enforcement layer is contacted — the denied tool never runs and no cost is charged
...
For cross-process and cross-machine enforcement, use the [governance server](/v0.2/guides/governance-server).
