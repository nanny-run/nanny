...
Every run emits [NDJSON events](/v0.2/concepts/event-log) to stdout. `ExecutionStarted` is always first; `ExecutionStopped` is always last:
...
For per-function governance (marking individual tools and rules in code), see the [Rust SDK guide](/v0.2/guides/rust-sdk) or [Python SDK guide](/v0.2/guides/python-sdk).
