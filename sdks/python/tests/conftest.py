...
sets ``NANNY_BRIDGE_ADDR`` so _client routes to it. Because _client reads
...
def mock_bridge(httpserver: HTTPServer, monkeypatch: pytest.MonkeyPatch) -> HTTPServer:
    ...
    Sets ``NANNY_BRIDGE_ADDR`` and ``NANNY_SESSION_TOKEN`` for the duration
    ...
    """
    monkeypatch.setenv("NANNY_BRIDGE_ADDR", f"127.0.0.1:{httpserver.port}")
